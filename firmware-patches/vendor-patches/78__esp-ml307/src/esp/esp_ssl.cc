...
bool EspSsl::Connect(const std::string& host, int port) {
    ...
    // 只设发送超时(不设接收超时)：send() 底层不设超时可无限阻塞主任务(网络卡顿/后端连接被
    // 强断时实测复现)，一旦冻结只能拔电源；但接收侧不能设超时——暖待命长时间无数据是正常状态
    // (设备空闲仍保持连接以待推送提醒)，误加接收超时会把它当断线，误杀这个功能。
    int sockfd;
    if (esp_tls_get_conn_sockfd(tls_client_, &sockfd) == ESP_OK && sockfd >= 0) {
        struct timeval send_timeout = { .tv_sec = 8, .tv_usec = 0 };
